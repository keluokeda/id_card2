...
//            }
    ...
    private fun pickImage() {
        ...
        if (activityInfo != null) {
            ...
            resultSubject.onNext(RecognitionResult.RecognitionError("无法打开相册软件"))
    ...
    override fun onRequestPermissionsResult(
        ...
        if (grantResult) {
            ...
            resultSubject.onNext(RecognitionResult.RecognitionError("用户没有授权"))
            resultSubject.onComplete()
        }
    }
        ...
        if (requestCode == request_code_camera) {
            ...
            if (resultCode == Activity.RESULT_OK) {
                ...
                val recognitionResult = data?.getSerializableExtra("result")
                    ...
                    ?: RecognitionResult.RecognitionError(data?.getStringExtra("error") ?: "")
                ...
                resultSubject.onNext(RecognitionResult.RecognitionError("识别取消"))
                resultSubject.onComplete()
            }
    ...
    private fun getResultFromGallery(data: Intent?) {
        ...
        CardOcrRecogConfigure.getInstance()
            ...
            .setFlag(0)
            .setnCropType(0)
            .setSavePath(DefaultPicSavePath(requireContext(), true))
        ...
        val importRecog = ImportRecog(activity!!.applicationContext, object : IBaseReturnMessage {
            override fun scanOCRIdCardSuccess(p0: ResultMessage, p1: Array<out String>) {
                ...
                val recognitionResult =
                    if (type == CardsCameraActivity.RECOGNITION_TYPE_ID_CARD) RecognitionResult.IDCardResult.fromResultMessage(
                        p0
                    )
                    else RecognitionResult.VehicleLicenseResult.fromResultMessage(p0)
            ...
            override fun scanOCRIdCardError(p0: String, p1: Array<out String>?) {
                dialog.dismiss()

                resultSubject.onNext(RecognitionResult.RecognitionError(p0))
                resultSubject.onComplete()
            }
        ...
        Observable.just(imagePath)
            ...
            .map {
                importRecog.startImportRecogService(uri, requireContext().applicationContext)
                it
            }
            .subscribe()
    }
