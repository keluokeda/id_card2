...
public class CardsCameraActivity extends AppCompatActivity implements IScanReturnMessage, View.OnClickListener {
    public static final int RECOGNITION_TYPE_ID_CARD = 2;

    public static final int RECOGNITION_TYPE_VEHICLE_LICENSE = 6;

    public static Intent createIntent(int type, Context context) {
        CardOcrRecogConfigure.getInstance()
                .initLanguage(context.getApplicationContext())
                .setSaveCut(false)
                ...
                .setFlag(0)
                .setnCropType(0)
                .setSavePath(new DefaultPicSavePath(context.getApplicationContext(), true));

        return new Intent(context, CardsCameraActivity.class);
    }
    ...
    public void scanOCRIdCardSuccess(ResultMessage resultMessage, String[] picPath) {
        ...
        int mainId = CardOcrRecogConfigure.getInstance().nMainId;

        if (mainId == 2) {
            //身份证

            intent.putExtra("result", RecognitionResult.IDCardResult.Companion.fromResultMessage(resultMessage));

        } else if (mainId == 6) {
            //行驶证
            intent.putExtra("result", RecognitionResult.VehicleLicenseResult.Companion.fromResultMessage(resultMessage));
        }
